...
public class StorageNotification extends SystemUI {
            ...
            for (VolumeRecord volumeRecord : this.mStorageManager.getVolumeRecords()) {
                if (volumeRecord.getType() == 1) {
                    ...
                    if ((findVolumeByUuid == null || !findVolumeByUuid.isMountedWritable()) && !volumeRecord.isSnoozed()) {
                        String string = this.mContext.getString(17039970, new Object[]{volumeRecord.getNickname()});
                        String string2 = this.mContext.getString(17039969);
                        Notification.Builder extend = new Notification.Builder(this.mContext, NotificationChannels.STORAGE).setSmallIcon(17302802).setColor(this.mContext.getColor(17170460)).setContentTitle(string).setContentText(string2).setContentIntent(buildForgetPendingIntent(volumeRecord)).setStyle(new Notification.BigTextStyle().bigText(string2)).setVisibility(1).setLocalOnly(true).setCategory("sys").setDeleteIntent(buildSnoozeIntent(fsUuid)).extend(new Notification.TvExtender());
    ...
    public void onDiskScannedInternal(DiskInfo diskInfo, int i) {
        ...
        String string = this.mContext.getString(17040000, new Object[]{diskInfo.getDescription()});
        String string2 = this.mContext.getString(17039999, new Object[]{diskInfo.getDescription()});
    ...
    private void onPublicVolumeStateChangedInternal(VolumeInfo volumeInfo) {
        ...
        if (!volumeInfo.disk.isNonRemovable()) {
    ...
    private Notification onVolumeChecking(VolumeInfo volumeInfo) {
        ...
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17039967, new Object[]{disk.getDescription()}), this.mContext.getString(17039966, new Object[]{disk.getDescription()})).setCategory("progress").setOngoing(true).build();
    }
    ...
    private Notification onVolumeMounted(VolumeInfo volumeInfo) {
        ...
        if (!disk.isAdoptable() || findRecordByUuid.isInited()) {
            ...
            String string = this.mContext.getString(17039981, new Object[]{disk.getDescription()});
            PendingIntent buildBrowsePendingIntent = buildBrowsePendingIntent(volumeInfo);
            Notification.Builder category = buildNotificationBuilder(volumeInfo, description, string).addAction(new Notification.Action(17302423, this.mContext.getString(17039965), buildBrowsePendingIntent)).addAction(new Notification.Action(17302405, this.mContext.getString(17039994), buildUnmountPendingIntent(volumeInfo))).setContentIntent(buildBrowsePendingIntent).setCategory("sys");
            if (disk.isUsb()) {
        ...
        String string2 = this.mContext.getString(17039977, new Object[]{disk.getDescription()});
        PendingIntent buildInitPendingIntent = buildInitPendingIntent(volumeInfo);
        return buildNotificationBuilder(volumeInfo, description2, string2).addAction(new Notification.Action(17302808, this.mContext.getString(17039968), buildInitPendingIntent)).addAction(new Notification.Action(17302405, this.mContext.getString(17039994), buildUnmountPendingIntent(volumeInfo))).setContentIntent(buildInitPendingIntent).setDeleteIntent(buildSnoozeIntent(volumeInfo.getFsUuid())).build();
    }

    private Notification onVolumeEjecting(VolumeInfo volumeInfo) {
        DiskInfo disk = volumeInfo.getDisk();
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17039998, new Object[]{disk.getDescription()}), this.mContext.getString(17039997, new Object[]{disk.getDescription()})).setCategory("progress").setOngoing(true).build();
    }

    private Notification onVolumeUnmountable(VolumeInfo volumeInfo) {
        DiskInfo disk = volumeInfo.getDisk();
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17039996, new Object[]{disk.getDescription()}), this.mContext.getString(17039995, new Object[]{disk.getDescription()})).setContentIntent(buildInitPendingIntent(volumeInfo)).setCategory("err").build();
    }
    ...
    private Notification onVolumeRemoved(VolumeInfo volumeInfo) {
        ...
        DiskInfo disk = volumeInfo.getDisk();
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17039980, new Object[]{disk.getDescription()}), this.mContext.getString(17039979, new Object[]{disk.getDescription()})).setCategory("err").build();
    }
    ...
    private Notification onVolumeBadRemoval(VolumeInfo volumeInfo) {
        ...
        DiskInfo disk = volumeInfo.getDisk();
        return buildNotificationBuilder(volumeInfo, this.mContext.getString(17039964, new Object[]{disk.getDescription()}), this.mContext.getString(17039963, new Object[]{disk.getDescription()})).setCategory("err").build();
    }
    ...
    public void onMoveProgress(MoveInfo moveInfo, int i, long j) {
        ...
        if (!TextUtils.isEmpty(moveInfo.label)) {
            str = this.mContext.getString(17039973, new Object[]{moveInfo.label});
        } else {
            str = this.mContext.getString(17039976);
        }
        if (j < 0) {
            charSequence = null;
        } else {
        ...
        Notification.Builder ongoing = new Notification.Builder(this.mContext, NotificationChannels.STORAGE).setSmallIcon(17302802).setColor(this.mContext.getColor(17170460)).setContentTitle(str).setContentText(charSequence).setContentIntent(pendingIntent).setStyle(new Notification.BigTextStyle().bigText(charSequence)).setVisibility(1).setLocalOnly(true).setCategory("progress").setProgress(100, i, false).setOngoing(true);
    ...
    public void onMoveFinished(MoveInfo moveInfo, int i) {
        ...
        if (i == -100) {
            str = this.mContext.getString(17039975);
            str2 = this.mContext.getString(17039974, new Object[]{bestVolumeDescription});
        } else {
            str = this.mContext.getString(17039972);
            str2 = this.mContext.getString(17039971);
        }
        ...
        Notification.Builder autoCancel = new Notification.Builder(this.mContext, NotificationChannels.STORAGE).setSmallIcon(17302802).setColor(this.mContext.getColor(17170460)).setContentTitle(str).setContentText(str2).setContentIntent(pendingIntent).setStyle(new Notification.BigTextStyle().bigText(str2)).setVisibility(1).setLocalOnly(true).setCategory("sys").setAutoCancel(true);
