...
public class ForcedResizableInfoActivity extends Activity implements View.OnTouchListener {
    ...
    public void onCreate(Bundle bundle) {
        ...
        setContentView(C1779R$layout.forced_resizable_activity);
        TextView textView = (TextView) findViewById(16908299);
        int intExtra = getIntent().getIntExtra("extra_forced_resizeable_reason", -1);
        if (intExtra == 1) {
            str = getString(C1784R$string.dock_forced_resizable);
        } else if (intExtra == 2) {
            str = getString(C1784R$string.forced_resizable_secondary_display);
        } else {
    ...
    public void finish() {
        super.finish();
        overridePendingTransition(0, C1770R$anim.forced_resizable_exit);
    }
}
