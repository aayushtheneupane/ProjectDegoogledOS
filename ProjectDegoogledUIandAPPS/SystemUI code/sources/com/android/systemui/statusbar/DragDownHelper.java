...
public class DragDownHelper implements Gefingerpoken {
    ...
    public DragDownHelper(final Context context, View view, ExpandHelper.Callback callback, DragDownCallback dragDownCallback, FalsingManager falsingManager) {
        ...
        this.mDoubleTapTimeout = (long) ViewConfiguration.getDoubleTapTimeout();
        this.mCallback = callback;
        this.mDragDownCallback = dragDownCallback;
        this.mHost = view;
        this.mFalsingManager = falsingManager;
        this.mStatusBarHeaderHeight = context.getResources().getDimensionPixelSize(C1775R$dimen.status_bar_header_height_keyguard);
        this.mGoToSleep = new Runnable() {
            public void run() {
    ...
    public boolean onInterceptTouchEvent(MotionEvent motionEvent) {
        ...
        if (actionMasked == 0) {
            ...
            if (this.mDoubleTapToSleepEnabled && y < ((float) this.mStatusBarHeaderHeight)) {
                long eventTime = motionEvent.getEventTime();
                long j = this.mLastDownEvent;
                if (j != -1) {
                    if (eventTime - j < this.mDoubleTapTimeout) {
                        this.mGoToSleep.run();
                    }
                    this.mLastDownEvent = -1;
                } else {
                    this.mLastDownEvent = eventTime;
                }
            }
