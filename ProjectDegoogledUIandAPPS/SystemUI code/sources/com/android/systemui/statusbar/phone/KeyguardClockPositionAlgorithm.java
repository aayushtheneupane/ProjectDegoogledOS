...
public class KeyguardClockPositionAlgorithm {
    ...
    public void loadDimens(Resources resources) {
        this.mClockNotificationsMargin = resources.getDimensionPixelSize(C1775R$dimen.keyguard_clock_notifications_margin);
        this.mContainerTopPadding = Math.max(resources.getDimensionPixelSize(C1775R$dimen.keyguard_clock_top_margin), resources.getDimensionPixelSize(C1775R$dimen.keyguard_lock_height) + resources.getDimensionPixelSize(C1775R$dimen.keyguard_lock_padding) + resources.getDimensionPixelSize(C1775R$dimen.keyguard_clock_lock_margin));
    ...
    private int getClockY(float f) {
        ...
        if (this.mHasCustomClock) {
            i = getPreferredAlternativeClockY(getMaxClockY());
        } else {
            i = getMaxClockY();
        }
        float max = MathUtils.max(0.0f, ((float) i) + burnInPreventionOffsetY());
