...
public class GridContent extends SliceContent {
    ...
    private int mLargestImageMode = 3;
    ...
    private boolean populate(SliceItem sliceItem) {
        ...
        SliceItem sliceItem2 = this.mSeeMoreItem;
        if (sliceItem2 != null && "slice".equals(sliceItem2.getFormat()) && (items = this.mSeeMoreItem.getSlice().getItems()) != null && items.size() > 0) {
        ...
        if ("slice".equals(sliceItem.getFormat())) {
            ...
            for (int i = 0; i < filterAndProcessItems.size(); i++) {
                SliceItem sliceItem3 = filterAndProcessItems.get(i);
                if (!"content_description".equals(sliceItem3.getSubType())) {
            ...
            if (!sliceItem.hasHint("shortcut") && ("slice".equals(format) || "action".equals(format))) {
                ...
                for (int i2 = 0; i2 < items.size(); i2++) {
                    ...
                    if ("content_description".equals(sliceItem2.getSubType())) {
                        ...
                        int i3 = 2;
                        if (this.mTextCount < 2 && ("text".equals(format2) || "long".equals(format2))) {
                            this.mTextCount++;
