...
public class PowerWhitelistBackend {
    ...
    public boolean isWhitelisted(String[] strArr) {
        ...
        for (String isWhitelisted : strArr) {
            if (isWhitelisted(isWhitelisted)) {
                return true;
            }
        }
        return false;
        ...
        if (iDeviceIdleController != null) {
            try {
                for (String add : iDeviceIdleController.getFullPowerWhitelist()) {
                    this.mWhitelistedApps.add(add);
                }
                for (String add2 : this.mDeviceIdleService.getSystemPowerWhitelist()) {
                    this.mSysWhitelistedApps.add(add2);
                }
                for (String add3 : this.mDeviceIdleService.getSystemPowerWhitelistExceptIdle()) {
                    this.mSysWhitelistedAppsExceptIdle.add(add3);
                }
