...
public class LockAfterTimeoutPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, Preference.OnPreferenceChangeListener {
    ...
    private void updateLockAfterPreferenceSummary(TimeoutListPreference timeoutListPreference) {
        ...
        if (timeoutListPreference.isDisabledByAdmin()) {
            charSequence = this.mContext.getText(C1715R.string.disabled_by_policy_title);
        } else {
            ...
            if (TextUtils.isEmpty(activeTrustAgentLabel)) {
                charSequence = this.mContext.getString(C1715R.string.lock_after_timeout_summary, new Object[]{entries[i]});
            } else if (Long.valueOf(entryValues[i].toString()).longValue() == 0) {
                charSequence = this.mContext.getString(C1715R.string.lock_immediately_summary_with_exception, new Object[]{activeTrustAgentLabel});
            } else {
                charSequence = this.mContext.getString(C1715R.string.lock_after_timeout_summary_with_exception, new Object[]{entries[i], activeTrustAgentLabel});
            }
        }
