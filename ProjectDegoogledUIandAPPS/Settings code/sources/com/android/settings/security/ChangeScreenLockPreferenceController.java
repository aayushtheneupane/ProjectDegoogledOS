...
public class ChangeScreenLockPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, GearPreference.OnGearClickListener {
    ...
    public boolean isAvailable() {
        return this.mContext.getResources().getBoolean(C1715R.bool.config_show_unlock_set_or_change);
    }
    ...
    public void updateSummary(Preference preference, int i) {
        if (this.mLockPatternUtils.isSecure(i)) {
            ...
            if (keyguardStoredPasswordQuality == 65536) {
                preference.setSummary((int) C1715R.string.unlock_set_unlock_mode_pattern);
            } else if (keyguardStoredPasswordQuality == 131072 || keyguardStoredPasswordQuality == 196608) {
                preference.setSummary((int) C1715R.string.unlock_set_unlock_mode_pin);
                ...
                preference.setSummary((int) C1715R.string.unlock_set_unlock_mode_password);
            }
        } else if (i == this.mProfileChallengeUserId || this.mLockPatternUtils.isLockScreenDisabled(i)) {
            preference.setSummary((int) C1715R.string.unlock_set_unlock_mode_off);
        } else {
            preference.setSummary((int) C1715R.string.unlock_set_unlock_mode_none);
        }
