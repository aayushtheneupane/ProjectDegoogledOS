...
public class SimSelectNotification extends BroadcastReceiver {
    ...
    private void onEnableMmsDataRequest(Context context, Intent intent) {
        ...
        if (!((SubscriptionManager) context.getSystemService("telephony_subscription_service")).isActiveSubId(intExtra)) {
        ...
        if (intExtra2 == 0) {
            charSequence = context.getResources().getText(C1715R.string.enable_receiving_mms_notification_title);
        } else if (intExtra2 == 1) {
            charSequence = context.getResources().getText(C1715R.string.enable_sending_mms_notification_title);
        } else {
            Log.w("SimSelectNotification", "onEnableMmsDataRequest invalid request reason " + intExtra2);
            return;
        }
        TelephonyManager createForSubscriptionId = ((TelephonyManager) context.getSystemService("phone")).createForSubscriptionId(intExtra);
        if (createForSubscriptionId.isDataEnabledForApn(2)) {
            Log.w("SimSelectNotification", "onEnableMmsDataRequest MMS data already enabled on sub ID " + intExtra);
            return;
        }
        String string = context.getResources().getString(C1715R.string.enable_mms_notification_summary, new Object[]{createForSubscriptionId.getSimOperatorName()});
    ...
    private void createSimSelectNotification(Context context) {
        ...
        NotificationChannel notificationChannel = new NotificationChannel(SIM_SELECT_NOTIFICATION_CHANNEL, resources.getText(C1715R.string.sim_selection_channel_title), 2);
        Notification.Builder autoCancel = new Notification.Builder(context, SIM_SELECT_NOTIFICATION_CHANNEL).setSmallIcon(C1715R.C1717drawable.ic_sim_alert).setColor(context.getColor(C1715R.C1716color.sim_noitification)).setContentTitle(resources.getText(C1715R.string.sim_notification_title)).setContentText(resources.getText(C1715R.string.sim_notification_summary)).setAutoCancel(true);
    ...
    private void createEnableMmsNotification(Context context, CharSequence charSequence, CharSequence charSequence2, int i) {
        NotificationChannel notificationChannel = new NotificationChannel(ENABLE_MMS_NOTIFICATION_CHANNEL, context.getResources().getText(C1715R.string.enable_mms_notification_channel_title), 4);
        Notification.Builder autoCancel = new Notification.Builder(context, ENABLE_MMS_NOTIFICATION_CHANNEL).setSmallIcon(C1715R.C1717drawable.ic_settings_24dp).setColor(context.getColor(C1715R.C1716color.sim_noitification)).setContentTitle(charSequence).setContentText(charSequence2).setStyle(new Notification.BigTextStyle().bigText(charSequence2)).setAutoCancel(true);
    ...
    private void createSimCombinationWarningNotification(Context context, Intent intent) {
        ...
        if (stringExtra != null) {
            String string = resources.getString(C1715R.string.dual_cdma_sim_warning_notification_summary, new Object[]{stringExtra});
            NotificationChannel notificationChannel = new NotificationChannel(SIM_WARNING_NOTIFICATION_CHANNEL, resources.getText(C1715R.string.dual_cdma_sim_warning_notification_channel_title), 4);
            Notification.Builder autoCancel = new Notification.Builder(context, SIM_WARNING_NOTIFICATION_CHANNEL).setSmallIcon(C1715R.C1717drawable.ic_sim_alert).setColor(context.getColor(C1715R.C1716color.sim_noitification)).setContentTitle(resources.getText(C1715R.string.sim_combination_warning_notification_title)).setContentText(string).setStyle(new Notification.BigTextStyle().bigText(string)).setAutoCancel(true);
