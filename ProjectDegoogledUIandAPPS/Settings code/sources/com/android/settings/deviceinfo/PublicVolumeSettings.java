...
public class PublicVolumeSettings extends SettingsPreferenceFragment {
    ...
    public void onCreate(Bundle bundle) {
        ...
        this.mMount = buildAction(C1715R.string.storage_menu_mount);
        this.mUnmount = new Button(getActivity());
        this.mUnmount.setText(C1715R.string.storage_menu_unmount);
        this.mUnmount.setOnClickListener(this.mUnmountListener);
        this.mFormatPublic = buildAction(C1715R.string.storage_menu_format);
        if (this.mIsPermittedToAdopt) {
            this.mFormatPrivate = buildAction(C1715R.string.storage_menu_format_private);
        }
    }
    ...
    public void onActivityCreated(Bundle bundle) {
        ...
        if (isVolumeValid()) {
            int dimensionPixelSize = getResources().getDimensionPixelSize(C1715R.dimen.unmount_button_padding);
    ...
    public void update() {
        ...
        if (this.mVolume.isMountedReadable()) {
            ...
            this.mSummary.setSummary((CharSequence) getString(C1715R.string.storage_volume_used, Formatter.formatFileSize(activity, totalSpace)));
            this.mSummary.setPercent(freeSpace, totalSpace);
        }
        if (this.mVolume.getState() == 0) {
            addPreference(this.mMount);
        }
        if (!this.mDisk.isNonRemovable() && this.mVolume.isMountedReadable()) {
