...
public class OverlayCategoryPreferenceController extends DeveloperOptionsPreferenceController implements Preference.OnPreferenceChangeListener, PreferenceControllerMixin {
    private static final Comparator<OverlayInfo> OVERLAY_INFO_COMPARATOR = Comparator.comparingInt(C0726x6d44a258.INSTANCE);
    ...
    private boolean setOverlay(final String str) {
        final String str2 = (String) getOverlayInfos().stream().filter(C0725xaccde52e.INSTANCE).map(C0727x1bccc31e.INSTANCE).findFirst().orElse((Object) null);
        ...
        new AsyncTask<Void, Void, Boolean>() {
            ...
            public void onPostExecute(Boolean bool) {
                ...
                if (!bool.booleanValue()) {
                    Toast.makeText(OverlayCategoryPreferenceController.this.mContext, C1715R.string.overlay_toast_failed_to_apply, 1).show();
                }
            }
        }.execute(new Void[0]);
        return true;
    ...
    public void updateState(Preference preference) {
        ...
        String string = this.mContext.getString(C1715R.string.overlay_option_device_default);
