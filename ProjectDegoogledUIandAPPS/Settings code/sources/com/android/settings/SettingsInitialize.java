...
public class SettingsInitialize extends BroadcastReceiver {
    ...
    private void managedProfileSetup(Context context, PackageManager packageManager, Intent intent, UserInfo userInfo) {
        ...
        if (userInfo != null && userInfo.isManagedProfile()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (!(resolveInfo.filter == null || (activityInfo = resolveInfo.activityInfo) == null || activityInfo.metaData == null || !resolveInfo.activityInfo.metaData.getBoolean("com.android.settings.PRIMARY_PROFILE_CONTROLLED"))) {
    ...
    public void refreshExistingShortcuts(Context context) {
        ...
        for (ShortcutInfo next : pinnedShortcuts) {
            if (!next.isImmutable()) {
                Intent intent = next.getIntent();
                intent.setFlags(335544320);
                arrayList.add(new ShortcutInfo.Builder(context, next.getId()).setIntent(intent).build());
            }
        }
