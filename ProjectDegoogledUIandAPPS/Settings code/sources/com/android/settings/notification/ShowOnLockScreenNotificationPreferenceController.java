...
public class ShowOnLockScreenNotificationPreferenceController extends AbstractPreferenceController implements PreferenceControllerMixin, Preference.OnPreferenceChangeListener {
    ...
    public void updateState(Preference preference) {
        ...
        String string = this.mContext.getString(C1715R.string.lock_screen_notifs_show_all);
        String num = Integer.toString(C1715R.string.lock_screen_notifs_show_all);
        arrayList.add(string);
        arrayList2.add(num);
        setRestrictedIfNotificationFeaturesDisabled(restrictedListPreference, string, num, 4);
        String string2 = this.mContext.getString(C1715R.string.lock_screen_notifs_show_alerting);
        String num2 = Integer.toString(C1715R.string.lock_screen_notifs_show_alerting);
        arrayList.add(string2);
        arrayList2.add(num2);
        setRestrictedIfNotificationFeaturesDisabled(restrictedListPreference, string2, num2, 4);
        arrayList.add(this.mContext.getString(C1715R.string.lock_screen_notifs_show_none));
        arrayList2.add(Integer.toString(C1715R.string.lock_screen_notifs_show_none));
        ...
        if (!adminAllowsNotifications() || !getLockscreenNotificationsEnabled()) {
            restrictedListPreference.setValue(Integer.toString(C1715R.string.lock_screen_notifs_show_none));
        } else if (!getLockscreenSilentNotificationsEnabled()) {
            restrictedListPreference.setValue(Integer.toString(C1715R.string.lock_screen_notifs_show_alerting));
        } else {
            restrictedListPreference.setValue(Integer.toString(C1715R.string.lock_screen_notifs_show_all));
        }
    ...
    public CharSequence getSummary() {
        if (!adminAllowsNotifications() || !getLockscreenNotificationsEnabled()) {
            return this.mContext.getString(C1715R.string.lock_screen_notifs_show_none);
        }
        if (!getLockscreenSilentNotificationsEnabled()) {
            return this.mContext.getString(C1715R.string.lock_screen_notifs_show_alerting);
        }
        return this.mContext.getString(C1715R.string.lock_screen_notifs_show_all);
    }

    public boolean onPreferenceChange(Preference preference, Object obj) {
        int parseInt = Integer.parseInt((String) obj);
        int i = 0;
        int i2 = parseInt != C1715R.string.lock_screen_notifs_show_none ? 1 : 0;
        if (parseInt == C1715R.string.lock_screen_notifs_show_all) {
            i = 1;
        }
