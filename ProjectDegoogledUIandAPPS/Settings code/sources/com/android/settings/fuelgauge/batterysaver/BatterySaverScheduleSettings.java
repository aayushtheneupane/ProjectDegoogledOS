...
public class BatterySaverScheduleSettings extends RadioButtonPickerFragment {
    ...
    public int getPreferenceScreenResId() {
        return C1715R.xml.battery_saver_schedule_settings;
    }

    public void onAttach(Context context) {
        super.onAttach(context);
        this.mSeekBarController = new BatterySaverScheduleSeekBarController(context);
        this.mRadioButtonController = new BatterySaverScheduleRadioButtonsController(context, this.mSeekBarController);
    ...
    public List<? extends CandidateInfo> getCandidates() {
        ...
        String string = getContext().getResources().getString(17039713);
        newArrayList.add(new BatterySaverScheduleCandidateInfo(context.getText(C1715R.string.battery_saver_auto_no_schedule), (CharSequence) null, "key_battery_saver_no_schedule", true));
        if (!TextUtils.isEmpty(string)) {
            newArrayList.add(new BatterySaverScheduleCandidateInfo(context.getText(C1715R.string.battery_saver_auto_routine), context.getText(C1715R.string.battery_saver_auto_routine_summary), "key_battery_saver_routine", true));
        } else {
            BatterySaverUtils.revertScheduleToNoneIfNeeded(context);
        }
        newArrayList.add(new BatterySaverScheduleCandidateInfo(context.getText(C1715R.string.battery_saver_auto_percentage), (CharSequence) null, "key_battery_saver_percentage", true));
        return newArrayList;
    }
