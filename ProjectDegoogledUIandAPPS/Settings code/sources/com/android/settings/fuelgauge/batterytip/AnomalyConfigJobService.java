...
public class AnomalyConfigJobService extends JobService {
    ...
    public static void scheduleConfigUpdate(Context context) {
        ...
        JobInfo.Builder persisted = new JobInfo.Builder(C1715R.integer.job_anomaly_config_update, new ComponentName(context, AnomalyConfigJobService.class)).setPeriodic(CONFIG_UPDATE_FREQUENCY_MS).setRequiresDeviceIdle(true).setRequiresCharging(true).setPersisted(true);
        if (jobScheduler.getPendingJob(C1715R.integer.job_anomaly_config_update) == null && jobScheduler.schedule(persisted.build()) != 1) {
            Log.i("AnomalyConfigJobService", "Anomaly config update job service schedule failed.");
        }
    }

    public boolean onStartJob(JobParameters jobParameters) {
        ThreadUtils.postOnBackgroundThread((Runnable) new Runnable(jobParameters) {
