...
public class FooterBarMixin implements Mixin {
    ...
    private int getPartnerTheme(FooterButton footerButton, int i, PartnerConfig partnerConfig) {
        ...
        if (!this.applyPartnerResources || color != 0) {
            return (!this.applyPartnerResources || color == 0) ? i : R$style.SucPartnerCustomizationButton_Primary;
        }
    ...
    private void onFooterButtonApplyPartnerResource(Button button, FooterButtonPartnerConfig footerButtonPartnerConfig) {
        if (this.applyPartnerResources) {
            ...
            updateButtonBackgroundWithPartnerConfig(button, footerButtonPartnerConfig.getButtonBackgroundConfig());
    ...
    @TargetApi(29)
    private void updateButtonBackgroundWithPartnerConfig(Button button, PartnerConfig partnerConfig) {
        ...
        if (color != 0) {
            ...
            obtainStyledAttributes.recycle();
            ColorStateList colorStateList = new ColorStateList(new int[][]{iArr, iArr2}, new int[]{convertRgbToArgb(color, f), color});
    ...
    private void updateButtonRippleColorWithPartnerConfig(Button button, FooterButtonPartnerConfig footerButtonPartnerConfig) {
        ...
        if (Build.VERSION.SDK_INT >= 21 && (rippleDrawable = getRippleDrawable(button)) != null) {
            int color = PartnerConfigHelper.get(this.context).getColor(this.context, footerButtonPartnerConfig.getButtonTextColorConfig());
            float fraction = PartnerConfigHelper.get(this.context).getFraction(this.context, footerButtonPartnerConfig.getButtonRippleColorAlphaConfig());
