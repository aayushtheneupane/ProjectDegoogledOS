...
public final class Palette {
    ...
    private float generateScore(Swatch swatch, Target target) {
        ...
        float f = ContactPhotoManager.OFFSET_DEFAULT;
        float abs = saturationWeight > ContactPhotoManager.OFFSET_DEFAULT ? (1.0f - Math.abs(hsl[1] - target.getTargetSaturation())) * target.getSaturationWeight() : ContactPhotoManager.OFFSET_DEFAULT;
        float lightnessWeight = target.getLightnessWeight() > ContactPhotoManager.OFFSET_DEFAULT ? target.getLightnessWeight() * (1.0f - Math.abs(hsl[2] - target.getTargetLightness())) : ContactPhotoManager.OFFSET_DEFAULT;
        if (target.getPopulationWeight() > ContactPhotoManager.OFFSET_DEFAULT) {
