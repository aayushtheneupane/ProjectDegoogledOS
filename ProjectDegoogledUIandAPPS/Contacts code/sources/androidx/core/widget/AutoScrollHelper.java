...
public abstract class AutoScrollHelper implements View.OnTouchListener {
    ...
    private float[] mMinimumVelocity = {ContactPhotoManager.OFFSET_DEFAULT, ContactPhotoManager.OFFSET_DEFAULT};
    boolean mNeedsCancel;
    boolean mNeedsReset;
    private float[] mRelativeEdges = {ContactPhotoManager.OFFSET_DEFAULT, ContactPhotoManager.OFFSET_DEFAULT};
    private float[] mRelativeVelocity = {ContactPhotoManager.OFFSET_DEFAULT, ContactPhotoManager.OFFSET_DEFAULT};
    ...
    private float computeTargetVelocity(int i, float f, float f2, float f3) {
        ...
        int i2 = (edgeValue > ContactPhotoManager.OFFSET_DEFAULT ? 1 : (edgeValue == ContactPhotoManager.OFFSET_DEFAULT ? 0 : -1));
        if (i2 == 0) {
    ...
    private float getEdgeValue(float f, float f2, float f3, float f4) {
        float f5;
        float constrain = constrain(f * f2, (float) ContactPhotoManager.OFFSET_DEFAULT, f3);
        float constrainEdgeValue = constrainEdgeValue(f2 - f4, constrain) - constrainEdgeValue(f4, constrain);
        if (constrainEdgeValue < ContactPhotoManager.OFFSET_DEFAULT) {
            f5 = -this.mEdgeInterpolator.getInterpolation(-constrainEdgeValue);
        } else if (constrainEdgeValue <= ContactPhotoManager.OFFSET_DEFAULT) {
            return ContactPhotoManager.OFFSET_DEFAULT;
        } else {
            f5 = this.mEdgeInterpolator.getInterpolation(constrainEdgeValue);
        }
    ...
    private float constrainEdgeValue(float f, float f2) {
        ...
        } else if (i == 2 && f < ContactPhotoManager.OFFSET_DEFAULT) {
            return f / (-f2);
        }
        return ContactPhotoManager.OFFSET_DEFAULT;
    }
    ...
    private static class ClampedScroller {
        ...
        private float getValueAt(long j) {
            ...
            if (j2 < 0 || j < j2) {
                return AutoScrollHelper.constrain(((float) (j - this.mStartTime)) / ((float) this.mRampUpDuration), (float) ContactPhotoManager.OFFSET_DEFAULT, 1.0f) * 0.5f;
            }
            long j3 = j - j2;
            float f = this.mStopValue;
            return (1.0f - f) + (f * AutoScrollHelper.constrain(((float) j3) / ((float) this.mEffectiveRampDown), (float) ContactPhotoManager.OFFSET_DEFAULT, 1.0f));
        }
