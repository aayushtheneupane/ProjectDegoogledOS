...
public class VectorDrawableCompat extends VectorDrawableCommon {
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.mTmpBounds.width() > 0 && this.mTmpBounds.height() > 0) {
            ...
            if (!(abs3 == ContactPhotoManager.OFFSET_DEFAULT && abs4 == ContactPhotoManager.OFFSET_DEFAULT)) {
                abs = 1.0f;
                abs2 = 1.0f;
            }
            ...
            if (min > 0 && min2 > 0) {
                ...
                if (needMirroring()) {
                    canvas.translate((float) this.mTmpBounds.width(), ContactPhotoManager.OFFSET_DEFAULT);
    ...
    private void updateStateFromTypedArray(TypedArray typedArray, XmlPullParser xmlPullParser, Resources.Theme theme) throws XmlPullParserException {
        ...
        if (vPathRenderer.mViewportWidth <= ContactPhotoManager.OFFSET_DEFAULT) {
            throw new XmlPullParserException(typedArray.getPositionDescription() + "<vector> tag requires viewportWidth > 0");
        } else if (vPathRenderer.mViewportHeight > ContactPhotoManager.OFFSET_DEFAULT) {
            ...
            if (vPathRenderer.mBaseWidth <= ContactPhotoManager.OFFSET_DEFAULT) {
                throw new XmlPullParserException(typedArray.getPositionDescription() + "<vector> tag requires width > 0");
            } else if (vPathRenderer.mBaseHeight > ContactPhotoManager.OFFSET_DEFAULT) {
    ...
    private static class VPathRenderer {
        ...
        public VPathRenderer() {
            ...
            this.mBaseWidth = ContactPhotoManager.OFFSET_DEFAULT;
            this.mBaseHeight = ContactPhotoManager.OFFSET_DEFAULT;
            this.mViewportWidth = ContactPhotoManager.OFFSET_DEFAULT;
            this.mViewportHeight = ContactPhotoManager.OFFSET_DEFAULT;
            this.mRootAlpha = 255;
            this.mRootName = null;
        ...
        private void drawPath(VGroup vGroup, VPath vPath, Canvas canvas, int i, int i2, ColorFilter colorFilter) {
            ...
            if (matrixScale != ContactPhotoManager.OFFSET_DEFAULT) {
                ...
                if (!(vFullPath.mTrimPathStart == ContactPhotoManager.OFFSET_DEFAULT && vFullPath.mTrimPathEnd == 1.0f)) {
                    ...
                    if (f7 > f8) {
                        ...
                        this.mPathMeasure.getSegment(ContactPhotoManager.OFFSET_DEFAULT, f8, path, true);
                    } else {
                        this.mPathMeasure.getSegment(f7, f8, path, true);
                    }
                    path.rLineTo(ContactPhotoManager.OFFSET_DEFAULT, ContactPhotoManager.OFFSET_DEFAULT);
                }
                this.mRenderPath.addPath(path, this.mFinalPathMatrix);
        ...
        private float getMatrixScale(Matrix matrix) {
            float[] fArr = {ContactPhotoManager.OFFSET_DEFAULT, 1.0f, 1.0f, ContactPhotoManager.OFFSET_DEFAULT};
            matrix.mapVectors(fArr);
            float cross = cross(fArr[0], fArr[1], fArr[2], fArr[3]);
            float max = Math.max((float) Math.hypot((double) fArr[0], (double) fArr[1]), (float) Math.hypot((double) fArr[2], (double) fArr[3]));
            if (max > ContactPhotoManager.OFFSET_DEFAULT) {
    ...
    private static class VGroup extends VObject {
        ...
        private float mPivotX = ContactPhotoManager.OFFSET_DEFAULT;
        private float mPivotY = ContactPhotoManager.OFFSET_DEFAULT;
        float mRotate = ContactPhotoManager.OFFSET_DEFAULT;
        private float mScaleX = 1.0f;
        private float mScaleY = 1.0f;
        final Matrix mStackedMatrix = new Matrix();
        private int[] mThemeAttrs;
        private float mTranslateX = ContactPhotoManager.OFFSET_DEFAULT;
        private float mTranslateY = ContactPhotoManager.OFFSET_DEFAULT;
        ...
        private void updateLocalMatrix() {
            ...
            this.mLocalMatrix.postRotate(this.mRotate, ContactPhotoManager.OFFSET_DEFAULT, ContactPhotoManager.OFFSET_DEFAULT);
    ...
    private static class VFullPath extends VPath {
        ...
        float mStrokeWidth = ContactPhotoManager.OFFSET_DEFAULT;
        private int[] mThemeAttrs;
        float mTrimPathEnd = 1.0f;
        float mTrimPathOffset = ContactPhotoManager.OFFSET_DEFAULT;
        float mTrimPathStart = ContactPhotoManager.OFFSET_DEFAULT;
